...
const createLiveEventRouter = require("./routes/liveEvent");
...
app.use("/api/live-event", createLiveEventRouter({
    admin,
...
app.post("/api/users/profile", verifyFirebaseUser, async (req, res) => {
    try {
        ...
        const email = String(req.body?.email || resolveVerifiedUserEmail(req.user) || "").trim().toLowerCase();
        ...
        const payload = {
            ...
            isVerified: Boolean(req.user?.email_verified || req.user?.emailVerified),
        ...
        console.error("Failed to create/update user profile:", {
            uid: resolveVerifiedUserUid(req.user),
            code: error?.code || "",
            message: error?.message || "",
            error
        });
        res.status(500).json({ message: error?.message || "Unable to create user profile." });
    }
});

app.post("/api/live-quiz-event/register", verifyFirebaseUser, async (req, res) => {
    try {
        const uid = resolveVerifiedUserUid(req.user);
        if (!uid) {
            res.status(401).json({ message: "Authenticated user is required." });
        ...
        const allowedClassLevels = new Set(["9", "10", "11", "12", "dropper", "other"]);
        const classLevel = String(req.body?.classLevel || "").trim().toLowerCase();
        const city = String(req.body?.city || "").trim();
        const source = String(req.body?.source || "").trim().toLowerCase();
        const confirmed = req.body?.confirmed === true;

        if (!allowedClassLevels.has(classLevel)) {
            res.status(400).json({ message: "A valid class is required." });
            return;
        }
        ...
        if (!confirmed) {
            res.status(400).json({ message: "Please confirm your details before submitting." });
            return;
        }

        const existingProfile = await getStudentProfile(uid);
        const existingName = String(existingProfile.name || existingProfile.fullName || "").trim();
        const fallbackName = String(req.user?.name || req.user?.displayName || "").trim()
            || String(resolveVerifiedUserEmail(req.user) || "").split("@")[0]
            || "Student";
        const mobile = String(existingProfile.mobile || existingProfile.phone || "").trim();
        const email = String(existingProfile.email || resolveVerifiedUserEmail(req.user) || "").trim().toLowerCase();

        const liveQuizEventData = {
            eventSlug: "live-quiz-event",
            confirmed: true,
            classLevel,
            city,
            source,
        ...
        if (!existingProfile?.liveQuizEvent?.registeredAt) {
            liveQuizEventData.registeredAt = admin.firestore.FieldValue.serverTimestamp();
        }
        ...
        try {
            await firestore.collection("users").doc(uid).set({
                ...
                fullName: existingName || fallbackName,
                email,
                mobile,
                phone: mobile,
                class: classLevel,
                ...
                role: existingProfile.role || "student",
                status: existingProfile.status || "active",
                liveQuizEvent: liveQuizEventData,
                updatedAt: admin.firestore.FieldValue.serverTimestamp(),
                createdAt: existingProfile.createdAt || admin.firestore.FieldValue.serverTimestamp()
            }, { merge: true });
        } catch (error) {
            console.error("Failed to store live quiz event user profile:", {
                uid,
                email,
                code: error?.code || "",
                message: error?.message || "",
        ...
        res.json({
            ...
            student: {
                uid,
                name: String(updatedProfile.name || existingName || fallbackName).trim(),
                email: String(updatedProfile.email || email).trim(),
                mobile: String(updatedProfile.mobile || updatedProfile.phone || mobile).trim()
            },
            registration: {
                classLevel,
                city,
        ...
        console.error("Failed to register live quiz event participant:", error);
        res.status(500).json({
            message: error?.message || "Unable to save the live quiz event registration."
        });
    }
});
